...
if (isset($_POST['actualizar']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($nickname) || empty($gender) || empty($user_type) || empty($badge_number)) {
        ...
        $stmt_check->execute([$badge_number, $adminId]);
        if ($stmt_check->fetchColumn() > 0) {
            ...
            try {
                ...
                if ($update_stmt->execute([$name, $nickname, $gender, $user_type, $badge_number, $adminId])) {
                    $success_message = "Dados do Admin actualizados com sucesso!";
                    header("Location: gerenciar_admin.php");
                    exit;
                } else {
                    $error_message = "Erro ao actualizar os dados do Admin.";
                }
...
if (isset($_POST['editar'])) {
    ...
    if ($row) {
        ...
        <head>
            <meta charset="UTF-8">
            <title>Actualizar dados do Admin | MuseiwaProg</title>
        ...
        <body>
            <div class="container">
                <h3>Actualizar dados do Admin</h3>
                <form action="actualizar_admin.php" method="post">
                    ...
                    <div class="form-row">
                        <div>
                            <label for="gender">Género do Admin:</label>
